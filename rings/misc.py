...
class FightError(Exception):
    ...
    def embed(self, bot):
        ...
        embed = discord.Embed(title="Fight Error", description=error_traceback, colour=self.bot.color)
...
class Misc(commands.Cog):
        ...
        async with self.bot.session.get('http://aws.random.cat/meow') as r:
            try:
                ...
                if self.bot.cat_cache:
                    await ctx.send("API overloading, have a cached picture instead.", embed=discord.Embed(colour=self.bot.color).set_image(url=random.choice(self.bot.cat_cache)))
                else:
    ...
    async def matchups(self, ctx, *, arguments = None):
        ...
        def embed_maker(index, entries):
            ...
            embed = discord.Embed(title=entries[0][0], description=intro + description, colour=self.bot.color)
    ...
    async def matchups_logs(self, ctx, *, args = None):
        ...
        def embed_maker(index, entries):
            ...
            embed = discord.Embed(title=f"Logs ({index[0]}/{index[1]})", description=description, colour=self.bot.color)
            embed.set_footer(**self.bot.bot_footer)
            return embed
