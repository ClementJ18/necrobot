...
class Bridge(commands.Cog):
    ...
    def __init__(self, bot):
        ...
        self.test_mapping = {
            "delthis": {"id": 778027866142146570, "url": "https://modding-union.com/index.php/topic,30439.0.html"} # test
        }
    ...
    async def on_raw_reaction_add(self, payload):
        ...
        if not payload.emoji.name in self.mapping and (not payload.emoji.name in self.test_mapping and payload.channel_id == self.TEST_CHANNEL):
            return
        ...
        post["url"] = self.mapping[payload.emoji.name]["url"] if payload.emoji.name in self.mapping else self.test_mapping[payload.emoji.name]["url"]
