...
class RSS(commands.Cog):
    ...
    async def twitch_sub_task(self):
        ...
        for stream in streams:
            if datetime.datetime.strptime(stream["started_at"].replace("Z", "+00:00"), "%Y-%m-%dT%H:%M:%S%z") <= last_time:
                continue

            embed = discord.Embed(title="Streamer Live", description=f"**{stream['user_name']}** has started streaming **[{stream['title']}](https://www.twitch.tv/{stream['user_login']})**")
    ...
    async def youtube_filters(self, ctx, youtuber_name : str, *, filters : str = ""):
        ...
        if filters == "":
            ...
            await ctx.send(f":white_check_mark: | Only videos with the words **{filters}** will be posted for **{youtuber_name}**")
    ...
    async def twitch_request(self, route, payload):
        ...
        async with self.bot.session.get(f"https://api.twitch.tv/helix/{route}", headers=headers, params=payload) as resp:
