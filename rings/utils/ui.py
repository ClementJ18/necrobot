...
class Confirm(discord.ui.View):
    ...
    async def on_timeout(self):
        ...
        self.stop()
        self.clear_items()
        await self.message.edit(content=f":negative_squared_cross_mark: | Interaction has expireded, please answer within **{self.timeout}** seconds.", view=self)
...
async def paginate(ctx, entries, page_size, embed_maker, *, timeout=300):
    ...
    paginator.message = await ctx.send(embed=paginator.embed_maker(paginator, paginator.get_entry_subset()), view=paginator)
...
class Paginator(discord.ui.View):
    ...
    async def first_page(self, interaction: discord.Interaction, button: discord.ui.Button):
        self.index = 0
        await interaction.response.edit_message(embed=self.embed_maker(self, self.get_entry_subset()), view=self)
